use bevy::app::Plugin;
use crate::{logger, prelude::AppState};
...
fn log_system_add_base<'a>(myname: &'static str, plugname: &str, schedule: &'static str, state: &'a str) {
    ...
    logger::one(
        ...
        &format!("Running with schedule {schedule}{state}: {myname_bare} (registered by: {plugname_bare})."),
    );
}

pub fn log_system_add_onenter<T: TrackedPlugin>(app_state: AppState, _myname: &'static str) {
    log_system_add_base(_myname, std::any::type_name::<T>(), "OnEnter ", app_state.as_ref())
}
