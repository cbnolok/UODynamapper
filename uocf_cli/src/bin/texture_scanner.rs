...
fn main() -> io::Result<()> {
                ...
                match process_dds(path, root) {
                    Ok(info) => {
                        if info.is_land_candidate == 1 {
                            ...
                            let target_path = PathBuf::from(land_dir).join(info.path.replace('/', "_"));
    ...
    infos.sort_by(|a, b| {
        a.width.cmp(&b.width).then(a.height.cmp(&b.height))
    });

    let csv_path = "/home/claudio/test/texture_info.csv";
    let mut wtr = csv::Writer::from_path(csv_path)?;

    wtr.write_record(&["path", "vpath_hash", "width", "height", "is_square", "is_opaque", "is_land_candidate", "size"])?;
    ...
    println!("Done. CSV saved to {}. Land candidates copied to {}", csv_path, land_dir);
...
fn process_dds(path: &Path, root: &str) -> io::Result<TextureInfo> {
    ...
    if &magic != b"DDS " {
        ...
        if present {
            ...
            if cursor.read_exact(&mut magic2).is_ok() && &magic2 == b"DDS " {
                ...
                return Err(io::Error::new(io::ErrorKind::InvalidData, "Expected DDS magic after EC header"));
            }
        } else {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "Texture not present in EC header"));
        }
    }
    ...
    let dds = ddsfile::Dds::read(&mut dds_cursor)
        .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, format!("DDS read error: {}", e)))?;
    ...
    let is_land_candidate = if is_square == 1 && is_opaque == 1 { 1 } else { 0 };
