...
- Normal-map source files can be DXT1 RGB DDS textures rather than BC5-style normal maps. DDS compression is not the runtime sampling format: packers decode and repack source pixels into UDDP atlas pages as RGBA8888 or BC7/BC7-RDO payloads. The shader samples the UDDP atlas RGB directly; the current convention uses red as tangent X, green as tangent Z, and blue as up before blending with the geometric/bicubic terrain normal.
