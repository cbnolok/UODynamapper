...
fn get_p(x: i32, y: i32, w: u32, h: u32, rgba: &[u8]) -> f32x4 {
...
fn set_tp(tx: u32, ty: u32, tw: u32, rgba: &mut [u8], color: f32x4) {
...
fn diff(c1: f32x4, c2: f32x4) -> f32 {
    let diff = (c1 - c2).abs().to_array();
    diff[0] + diff[1] + diff[2]
}

fn interpolate(c1: f32x4, c2: f32x4) -> f32x4 {
    (c1 + c2) * f32x4::splat(0.5)
}

fn interpolate4(c1: f32x4, c2: f32x4, c3: f32x4, c4: f32x4) -> f32x4 {
    (c1 + c2 + c3 + c4) * f32x4::splat(0.25)
}

fn get_tp_f(x: i32, y: i32, w: u32, h: u32, rgba: &[u8]) -> f32x4 {
    let px = x.clamp(0, w as i32 - 1) as usize;
    let py = y.clamp(0, h as i32 - 1) as usize;
    let idx = (py * w as usize + px) * 4;
    f32x4::from([
