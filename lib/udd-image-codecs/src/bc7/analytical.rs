...
fn to_5p(c: i32, pbit: i32) -> i32 {
    ...
    if (q6 & 1) == pbit {
        ...
        let rhs = 255 * q6;
        let q6 = if lhs >= rhs {
            if q6 < 63 { q6 + 1 } else { q6 - 1 }
        } else {
            if q6 > 0 { q6 - 1 } else { q6 + 1 }
        };
        q6 >> 1
    }
}
...
#[inline]
fn to_5_clamp(c: f32, pbit: u32) -> u32 {
    to_5p(rnd(c).clamp(0, 255), pbit as i32) as u32
}
/// Convert float 0–255 to 5-bit without p-bit.
#[inline]
fn to_5f(c: f32) -> i32 {
    to_5(rnd(c.clamp(0.0, 255.0)))
}

/// Convert 8-bit to 6-bit with p-bit constraint (7-bit rounded, then LSB forced).
#[inline]
fn to_6p(c: i32, pbit: i32) -> i32 {
    ...
    if (q7 & 1) == pbit {
        ...
        let rhs = 255 * q7;
        let q7 = if lhs >= rhs {
            if q7 < 127 { q7 + 1 } else { q7 - 1 }
        } else {
            if q7 > 0 { q7 - 1 } else { q7 + 1 }
        };
        q7 >> 1
    }
}
...
#[inline]
fn bc7_interp(lo: i32, hi: i32, w: u32) -> i32 {
    let d = hi - lo;
    lo + ((d * w as i32 + 32) >> 6)
}
...
#[inline]
fn bc7_sse1(p: i32, lo: i32, d: i32, w: u32) -> u32 {
...
pub fn ls_fit_1d(
    ...
    for i in 0..n {
        let t = &ls_tab[weights[i] as usize];
        z00 += t[0];
        z10 += t[1];
        z11 += t[2];
        q00 += t[3] * pixels[i][comp] as f32;
    }
    let q10 = total - q00;
    ...
    let inv = 1.0 / det;
    let (iz00, iz01, iz10, iz11) = (z11 * inv, -z10 * inv, -z10 * inv, z00 * inv);
    Some((
        (iz10 * q00 + iz11 * q10).clamp(0.0, 255.0),
        (iz00 * q00 + iz01 * q10).clamp(0.0, 255.0),
    ))
}
...
pub fn ls_fit_3d(
    ...
    let (mut z00, mut z10, mut z11) = (0.0f32, 0.0f32, 0.0f32);
    let (mut qr, mut qg, mut qb) = (0.0f32, 0.0f32, 0.0f32);
    for i in 0..n {
        let t = &ls_tab[weights[i] as usize];
        z00 += t[0];
        z10 += t[1];
        z11 += t[2];
        qr += t[3] * pixels[i][0] as f32;
        qg += t[3] * pixels[i][1] as f32;
        qb += t[3] * pixels[i][2] as f32;
    }
    let det = z00 * z11 - z10 * z10;
    if det.abs() < 1e-8 {
        return None;
    ...
    Some((
        [
            (iz10 * qr + iz11 * (tr - qr)).clamp(0.0, 255.0),
            (iz10 * qg + iz11 * (tg - qg)).clamp(0.0, 255.0),
            (iz10 * qb + iz11 * (tb - qb)).clamp(0.0, 255.0),
        ],
        [
            (iz00 * qr + iz01 * (tr - qr)).clamp(0.0, 255.0),
            (iz00 * qg + iz01 * (tg - qg)).clamp(0.0, 255.0),
            (iz00 * qb + iz01 * (tb - qb)).clamp(0.0, 255.0),
        ],
    ))
}
...
pub fn ls_fit_4d(
    ...
    let (mut z00, mut z10, mut z11) = (0.0f32, 0.0f32, 0.0f32);
    let (mut qr, mut qg, mut qb, mut qa) = (0.0f32, 0.0f32, 0.0f32, 0.0f32);
    for i in 0..n {
        let t = &ls_tab[weights[i] as usize];
        z00 += t[0];
        z10 += t[1];
    ...
    Some((
        [
            ...
            (iz10 * qg + iz11 * (tg - qg)).clamp(0.0, 255.0),
            (iz10 * qb + iz11 * (tb - qb)).clamp(0.0, 255.0),
            (iz10 * qa + iz11 * (ta - qa)).clamp(0.0, 255.0),
        ],
        [
            (iz00 * qr + iz01 * (tr - qr)).clamp(0.0, 255.0),
            (iz00 * qg + iz01 * (tg - qg)).clamp(0.0, 255.0),
            (iz00 * qb + iz01 * (tb - qb)).clamp(0.0, 255.0),
            (iz00 * qa + iz01 * (ta - qa)).clamp(0.0, 255.0),
        ],
    ))
}
...
// ─── 5. WEIGHT EVALUATION ─────────────────────────────────────────────────────
#[inline]
fn sse1(p: i32, lo: i32, d: i32, w: u32) -> u32 {
    let e = p - (lo + ((d * w as i32 + 32) >> 6));
    (e * e) as u32
}
#[inline]
fn sse3(px: &Pixel, lr: i32, lg: i32, lb: i32, dr: i32, dg: i32, db: i32, w: u32) -> u32 {
    sse1(px[0] as i32, lr, dr, w) + sse1(px[1] as i32, lg, dg, w) + sse1(px[2] as i32, lb, db, w)
}
#[inline]
fn sse4(
    px: &Pixel,
    ...
    sse1(px[0] as i32, lr, dr, w)
        + sse1(px[1] as i32, lg, dg, w)
        + sse1(px[2] as i32, lb, db, w)
        + sse1(px[3] as i32, la, da, w)
}

#[inline(always)]
fn clamp_weight_sel(sel: i32, max_w: i32) -> i32 {
    if sel as u32 > max_w as u32 {
        (!sel >> 31) & max_w
    } else {
        sel
    }
...
type EvalM6RgbBackend =
    unsafe fn(&[Pixel; 16], &mut [u8; 16], i32, i32, i32, i32, i32, i32, f32) -> u32;

#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
type EvalM6RgbaBackend =
    unsafe fn(&[Pixel; 16], &mut [u8; 16], i32, i32, i32, i32, i32, i32, i32, i32, f32) -> u32;
...
fn eval_rgba_partition_weights4(
    ...
    let scale = f32x4::from([f[subsets[0]], f[subsets[1]], f[subsets[2]], f[subsets[3]]]);
    let sel_f =
        ((pr * dr + pg * dg + pb * db + pa * da + bias) * scale + f32x4::splat(0.5)).to_array();
    let mut sse = 0u32;
    for lane in 0..4 {
        let s = subsets[lane];
        let sel = clamp_weight_sel(sel_f[lane] as i32, max_w);
...
unsafe fn sse_m6_rgb4_sse41(
    ...
    let recon_r = _mm_add_epi32(
        _mm_set1_epi32(lr),
        _mm_srai_epi32(
            _mm_add_epi32(_mm_mullo_epi32(_mm_set1_epi32(dr), w), half),
            6,
        ),
    );
    let recon_g = _mm_add_epi32(
        _mm_set1_epi32(lg),
        _mm_srai_epi32(
            _mm_add_epi32(_mm_mullo_epi32(_mm_set1_epi32(dg), w), half),
            6,
        ),
    );
    let recon_b = _mm_add_epi32(
        _mm_set1_epi32(lb),
        _mm_srai_epi32(
            _mm_add_epi32(_mm_mullo_epi32(_mm_set1_epi32(db), w), half),
            6,
        ),
    );
    ...
    let err = _mm_add_epi32(
        _mm_add_epi32(_mm_mullo_epi32(er, er), _mm_mullo_epi32(eg, eg)),
        _mm_mullo_epi32(eb, eb),
    );
...
unsafe fn sse_m6_rgba4_sse41(
    ...
    let recon_a = _mm_add_epi32(
        _mm_set1_epi32(la),
        _mm_srai_epi32(
            _mm_add_epi32(_mm_mullo_epi32(_mm_set1_epi32(da), w), half),
            6,
        ),
    );
    let er = _mm_sub_epi32(pr, recon_r);
    ...
    let err = _mm_add_epi32(
        _mm_add_epi32(_mm_mullo_epi32(er, er), _mm_mullo_epi32(eg, eg)),
        _mm_add_epi32(_mm_mullo_epi32(eb, eb), _mm_mullo_epi32(ea, ea)),
    );
    let sum2 = _mm_add_epi32(err, _mm_shuffle_epi32(err, 0b10_11_00_01));
    let sum4 = _mm_add_epi32(sum2, _mm_shuffle_epi32(sum2, 0b01_00_11_10));
    _mm_cvtsi128_si32(sum4) as u32
...
unsafe fn sse_m6_rgb4_neon(
    ...
    const G_MASK: [u8; 16] = [
        1, 5, 9, 13, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255,
    ];
    const B_MASK: [u8; 16] = [
        2, 6, 10, 14, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255,
    ];
    ...
    let recon_r = vaddq_s32(
        vdupq_n_s32(lr),
        vshrq_n_s32::<6>(vaddq_s32(vmulq_s32(vdupq_n_s32(dr), w), half)),
    );
    let recon_g = vaddq_s32(
        vdupq_n_s32(lg),
        vshrq_n_s32::<6>(vaddq_s32(vmulq_s32(vdupq_n_s32(dg), w), half)),
    );
    let recon_b = vaddq_s32(
        vdupq_n_s32(lb),
        vshrq_n_s32::<6>(vaddq_s32(vmulq_s32(vdupq_n_s32(db), w), half)),
    );
    let er = vsubq_s32(pr, recon_r);
    let eg = vsubq_s32(pg, recon_g);
    let eb = vsubq_s32(pb, recon_b);
    vaddvq_s32(vaddq_s32(
        vaddq_s32(vmulq_s32(er, er), vmulq_s32(eg, eg)),
        vmulq_s32(eb, eb),
    )) as u32
}
...
unsafe fn sse_m6_rgba4_neon(
    ...
    const A_MASK: [u8; 16] = [
        3, 7, 11, 15, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255,
    ];
    let pr = vreinterpretq_s32_u32(vmovl_u16(vget_low_u16(vmovl_u8(vget_low_u8(vqtbl1q_u8(
        px,
        vld1q_u8(R_MASK.as_ptr()),
    ...
    let recon_a = vaddq_s32(
        vdupq_n_s32(la),
        vshrq_n_s32::<6>(vaddq_s32(vmulq_s32(vdupq_n_s32(da), w), half)),
    );
    let er = vsubq_s32(pr, recon_r);
    let eg = vsubq_s32(pg, recon_g);
    let eb = vsubq_s32(pb, recon_b);
    ...
    vaddvq_s32(vaddq_s32(
        vaddq_s32(vmulq_s32(er, er), vmulq_s32(eg, eg)),
        vaddq_s32(vmulq_s32(eb, eb), vmulq_s32(ea, ea)),
    )) as u32
}
...
unsafe fn select_m6_avx512(pairs: __m512i, f: f32) -> u64 {
    ...
    let y = _mm512_add_ps(
        _mm512_mul_ps(_mm512_cvtepi32_ps(dots), _mm512_set1_ps(f)),
        _mm512_set1_ps(0.5),
    );
    let sel32 = _mm512_min_epi32(
        _mm512_max_epi32(_mm512_cvttps_epi32(y), _mm512_setzero_si512()),
...
unsafe fn eval_m6_rgb_avx512(
    ...
    for i in (0..16).step_by(8) {
        ...
        let packed1 = (packed >> 32) as u32;
        sse += unsafe {
            sse_m6_rgb4_sse41(_mm256_castsi256_si128(px), packed0, lr, lg, lb, dr, dg, db)
        };
        sse += unsafe {
            sse_m6_rgb4_sse41(
                _mm256_extracti128_si256::<1>(px),
...
unsafe fn eval_m6_rgba_avx2(
    ...
    for i in (0..16).step_by(8) {
        ...
        let dots0 = _mm_unpacklo_epi64(
            _mm256_castsi256_si128(sums0),
            _mm256_extracti128_si256::<1>(sums0),
        );
        let dots1 = _mm_unpacklo_epi64(
            _mm256_castsi256_si128(sums1),
            _mm256_extracti128_si256::<1>(sums1),
        );
        let dots = _mm256_set_m128i(dots1, dots0);
        let y = _mm256_add_ps(
            _mm256_mul_ps(_mm256_cvtepi32_ps(dots), _mm256_set_m128(f, f)),
            _mm256_set_m128(half, half),
        );
        let sel32 = _mm256_min_epi32(
            _mm256_max_epi32(_mm256_cvttps_epi32(y), _mm256_setzero_si256()),
            fifteen256,
        );
        let sel16 = _mm_packus_epi32(
            _mm256_castsi256_si128(sel32),
            _mm256_extracti128_si256::<1>(sel32),
        );
...
pub fn eval_m6_rgb(
    ...
    let (lr, lg, lb) = (
        from_7(lr as u32, p0) as i32,
        from_7(lg as u32, p0) as i32,
        from_7(lb as u32, p0) as i32,
    );
    let (hr, hg, hb) = (
        from_7(hr as u32, p1) as i32,
        from_7(hg as u32, p1) as i32,
        from_7(hb as u32, p1) as i32,
    );
    let (dr, dg, db) = (hr - lr, hg - lg, hb - lb);
    let f = 15.0 / ((dr * dr + dg * dg + db * db) as f32 + 1.25e-7);
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
...
pub fn eval_m6_rgba(
    ...
    let (lr, lg, lb, la) = (
        ...
        from_7(lg as u32, p0) as i32,
        from_7(lb as u32, p0) as i32,
        from_7(la as u32, p0) as i32,
    );
    let (hr, hg, hb, ha) = (
        from_7(hr as u32, p1) as i32,
        from_7(hg as u32, p1) as i32,
        from_7(hb as u32, p1) as i32,
        from_7(ha as u32, p1) as i32,
    );
    let (dr, dg, db, da) = (hr - lr, hg - lg, hb - lb, ha - la);
    let f = 15.0 / ((dr * dr + dg * dg + db * db + da * da) as f32 + 1.25e-7);
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
...
pub fn eval_m1(
    ...
    let mut el = [[0i32; 3]; 2];
    let mut dlt = [[0i32; 3]; 2];
    let mut f = [0.0f32; 2];
    let mut sofs = [0i32; 2];
    for s in 0..2 {
        el[s][0] = from_6(lr[s], pbits[s]) as i32;
        let ehr = from_6(hr[s], pbits[s]) as i32;
        el[s][1] = from_6(lg[s], pbits[s]) as i32;
        let ehg = from_6(hg[s], pbits[s]) as i32;
        el[s][2] = from_6(lb[s], pbits[s]) as i32;
        let ehb = from_6(hb[s], pbits[s]) as i32;
        dlt[s] = [ehr - el[s][0], ehg - el[s][1], ehb - el[s][2]];
        let d2 = dlt[s][0] * dlt[s][0] + dlt[s][1] * dlt[s][1] + dlt[s][2] * dlt[s][2];
        f[s] = 7.0 / (d2 as f32 + 1.25e-7);
        sofs[s] = el[s][0] * dlt[s][0] + el[s][1] * dlt[s][1] + el[s][2] * dlt[s][2];
    }
    let mut sse = 0u32;
    for i in (0..16).step_by(4) {
        sse += eval_rgb_partition_weights4(
            pixels,
            weights,
            i,
            [
                ((bmask >> i) & 1) as usize,
                ((bmask >> (i + 1)) & 1) as usize,
                ((bmask >> (i + 2)) & 1) as usize,
                ((bmask >> (i + 3)) & 1) as usize,
            ],
            &el,
            &dlt,
            &sofs,
...
pub fn eval_m3(
    ...
    let mut sofs = [0i32; 2];
    for s in 0..2 {
        el[s][0] = from_7(lr[s], pbits[s * 2]) as i32;
        let ehr = from_7(hr[s], pbits[s * 2 + 1]) as i32;
        el[s][1] = from_7(lg[s], pbits[s * 2]) as i32;
        let ehg = from_7(hg[s], pbits[s * 2 + 1]) as i32;
        el[s][2] = from_7(lb[s], pbits[s * 2]) as i32;
        let ehb = from_7(hb[s], pbits[s * 2 + 1]) as i32;
        dlt[s] = [ehr - el[s][0], ehg - el[s][1], ehb - el[s][2]];
        let d2 = dlt[s][0] * dlt[s][0] + dlt[s][1] * dlt[s][1] + dlt[s][2] * dlt[s][2];
        f[s] = 3.0 / (d2 as f32 + 1.25e-7);
        sofs[s] = el[s][0] * dlt[s][0] + el[s][1] * dlt[s][1] + el[s][2] * dlt[s][2];
    }
    let mut sse = 0u32;
    for i in (0..16).step_by(4) {
...
pub fn eval_m7(
    ...
    let mut el = [[0i32; 4]; 2];
    let mut dlt = [[0i32; 4]; 2];
    let mut f = [0.0f32; 2];
    let mut sofs = [0i32; 2];
    for s in 0..2 {
        el[s][0] = from_5p(lr[s], pbits[s * 2]) as i32;
        let ehr = from_5p(hr[s], pbits[s * 2 + 1]) as i32;
        el[s][1] = from_5p(lg[s], pbits[s * 2]) as i32;
        let ehg = from_5p(hg[s], pbits[s * 2 + 1]) as i32;
        el[s][2] = from_5p(lb[s], pbits[s * 2]) as i32;
        let ehb = from_5p(hb[s], pbits[s * 2 + 1]) as i32;
        el[s][3] = from_5p(la[s], pbits[s * 2]) as i32;
        let eha = from_5p(ha[s], pbits[s * 2 + 1]) as i32;
        dlt[s] = [
            ehr - el[s][0],
            ehg - el[s][1],
            ehb - el[s][2],
        ...
        let d2: i32 = dlt[s].iter().map(|&x| x * x).sum();
        f[s] = 3.0 / (d2 as f32 + 1.25e-7);
        sofs[s] = dlt[s].iter().zip(el[s].iter()).map(|(&d, &e)| d * e).sum();
    }
    let mut sse = 0u32;
    for i in (0..16).step_by(4) {
        sse += eval_rgba_partition_weights4(
            pixels,
            weights,
            i,
            [
...
pub fn eval_m0(
    ...
    let mut el = [[0i32; 3]; 3];
    let mut dlt = [[0i32; 3]; 3];
    let mut f = [0.0f32; 3];
    let mut sofs = [0i32; 3];
    for s in 0..3 {
        el[s][0] = from_4(lr[s], pbits[s * 2]) as i32;
        let ehr = from_4(hr[s], pbits[s * 2 + 1]) as i32;
        el[s][1] = from_4(lg[s], pbits[s * 2]) as i32;
        let ehg = from_4(hg[s], pbits[s * 2 + 1]) as i32;
        el[s][2] = from_4(lb[s], pbits[s * 2]) as i32;
        let ehb = from_4(hb[s], pbits[s * 2 + 1]) as i32;
        dlt[s] = [ehr - el[s][0], ehg - el[s][1], ehb - el[s][2]];
        let d2 = dlt[s][0] * dlt[s][0] + dlt[s][1] * dlt[s][1] + dlt[s][2] * dlt[s][2];
        f[s] = 7.0 / (d2 as f32 + 1.25e-7);
        sofs[s] = el[s][0] * dlt[s][0] + el[s][1] * dlt[s][1] + el[s][2] * dlt[s][2];
...
pub fn eval_m2(
    ...
    let mut sofs = [0i32; 3];
    for s in 0..3 {
        el[s][0] = from_5(lr[s]) as i32;
        let ehr = from_5(hr[s]) as i32;
        el[s][1] = from_5(lg[s]) as i32;
        let ehg = from_5(hg[s]) as i32;
        el[s][2] = from_5(lb[s]) as i32;
        let ehb = from_5(hb[s]) as i32;
        dlt[s] = [ehr - el[s][0], ehg - el[s][1], ehb - el[s][2]];
        let d2 = dlt[s][0] * dlt[s][0] + dlt[s][1] * dlt[s][1] + dlt[s][2] * dlt[s][2];
        f[s] = 3.0 / (d2 as f32 + 1.25e-7);
        sofs[s] = el[s][0] * dlt[s][0] + el[s][1] * dlt[s][1] + el[s][2] * dlt[s][2];
...
pub fn encode_mode0(
    ...
    if w[0] & 4 != 0 {
        inv[0] = 7;
        std::mem::swap(&mut lr[0], &mut hr[0]);
        std::mem::swap(&mut lg[0], &mut hg[0]);
        std::mem::swap(&mut lb[0], &mut hb[0]);
        {
            let _t = p[0];
            p[0] = p[1];
            p[1] = _t;
    ...
    if w[anc1] & 4 != 0 {
        inv[1] = 7;
        std::mem::swap(&mut lr[1], &mut hr[1]);
        std::mem::swap(&mut lg[1], &mut hg[1]);
        std::mem::swap(&mut lb[1], &mut hb[1]);
        {
            let _t = p[2];
            p[2] = p[3];
            p[3] = _t;
    ...
    if w[anc2] & 4 != 0 {
        inv[2] = 7;
        std::mem::swap(&mut lr[2], &mut hr[2]);
        std::mem::swap(&mut lg[2], &mut hg[2]);
        std::mem::swap(&mut lb[2], &mut hb[2]);
        {
            let _t = p[4];
            p[4] = p[5];
            p[5] = _t;
    ...
    for i in 0..16 {
        let s = pm[i] as usize;
        let ww = (w[i] ^ inv[s]) as u64;
        hi |= ww << ofs;
        ofs += 3 - ((i == 0 || i == anc1 || i == anc2) as usize);
    }
...
pub fn encode_mode1(
    ...
    if w[0] & 4 != 0 {
        inv[0] = 7;
        for c in 0..2 {
            std::mem::swap(&mut lr[c], &mut hr[c]);
            std::mem::swap(&mut lg[c], &mut hg[c]);
            std::mem::swap(&mut lb[c], &mut hb[c]);
        }
    ...
    let mut y = p0 as u64 | (p1 as u64) << 1;
    let mut ofs = 2usize;
    for i in 0..16 {
        let s = pm[i] as usize;
        let ww = (w[i] ^ inv[s]) as u64;
        y |= ww << ofs;
        ofs += 3 - ((i == 0 || i == anc) as usize);
    }
...
pub fn encode_mode2(
    ...
    let mut v1 = hg[2] as u64
        ...
        | (lb[2] as u64) << 25
        | (hb[2] as u64) << 30;
    block[8..12].copy_from_slice(&v1.to_le_bytes()[0..4]);
    v1 >>= 32;
    let mut ofs = 3usize;
    for i in 0..16 {
        let s = pm[i] as usize;
        let ww = (w[i] ^ inv[s]) as u64;
        v1 |= ww << ofs;
        ofs += 2 - ((i == 0 || i == anc1 || i == anc2) as usize);
    }
...
pub fn encode_mode6(
    ...
    if w[0] & 8 != 0 {
        inv = 15;
        std::mem::swap(&mut lr, &mut hr);
        std::mem::swap(&mut lg, &mut hg);
        std::mem::swap(&mut lb, &mut hb);
        std::mem::swap(&mut la, &mut ha);
        std::mem::swap(&mut p0, &mut p1);
    }
    let x = 0b1000000u64
        | (lr as u64) << 7
        ...
        | (la as u64) << 49
        | (ha as u64) << 56;
    block[0..7].copy_from_slice(&x.to_le_bytes()[0..7]);
    block[7] = (x >> 56) as u8 | (p0 as u8) << 7;
    let y = (p1 as u64)
        | ((w[0] ^ inv) as u64) << 1
        | ((w[1] ^ inv) as u64) << 4
        | ((w[2] ^ inv) as u64) << 8
...
pub fn encode_mode7(
    ...
    if w[0] & 2 != 0 {
        ...
        std::mem::swap(&mut lg[0], &mut hg[0]);
        std::mem::swap(&mut lb[0], &mut hb[0]);
        std::mem::swap(&mut la[0], &mut ha[0]);
        {
            let _t = p[0];
            p[0] = p[1];
            p[1] = _t;
    ...
    if w[anc] & 2 != 0 {
        ...
        std::mem::swap(&mut lg[1], &mut hg[1]);
        std::mem::swap(&mut lb[1], &mut hb[1]);
        std::mem::swap(&mut la[1], &mut ha[1]);
        {
            let _t = p[2];
            p[2] = p[3];
            p[3] = _t;
...
fn encode_solid_block(block: &mut [u8; 16], c: &Pixel) {
    let (lr, hr) = {
        let e = BC7_MODE5_OPTIMAL_ENDPOINTS[c[0] as usize];
        (e.0 as u64, e.1 as u64)
    };
    let (lg, hg) = {
        let e = BC7_MODE5_OPTIMAL_ENDPOINTS[c[1] as usize];
        (e.0 as u64, e.1 as u64)
    };
    let (lb, hb) = {
        let e = BC7_MODE5_OPTIMAL_ENDPOINTS[c[2] as usize];
        (e.0 as u64, e.1 as u64)
    };
...
fn part3_candidates(
    ...
    for pat in 0..64usize {
        ...
        for perm in PERMS {
            let diff: u32 = (0..16)
                .map(|i| (perm[pm[i] as usize] != desired[i]) as u32)
                .sum();
...
fn init_endpoints_m1_subset(
    ...
    for i in 0..16 {
        ...
        let d = pixels[i][0] as f32 * xr
            + pixels[i][1] as f32 * xg
            + pixels[i][2] as f32 * xb
            + i as f32 * 1e-4;
        if d < lo_dot {
            lo_dot = d;
            lo_i = i;
    ...
    (
        [
            to_6p(p[0] as i32, 0) as u32,
            to_6p(p[1] as i32, 0) as u32,
            to_6p(p[2] as i32, 0) as u32,
        ],
        [
            to_6p(q[0] as i32, 0) as u32,
            to_6p(q[1] as i32, 0) as u32,
            to_6p(q[2] as i32, 0) as u32,
        ],
    )
}
...
fn init_endpoints_m3_subset(
    ...
    (
        [
            to_7p(p[0] as i32, 0) as u32,
            to_7p(p[1] as i32, 0) as u32,
            to_7p(p[2] as i32, 0) as u32,
        ],
        [
            to_7p(q[0] as i32, 0) as u32,
            to_7p(q[1] as i32, 0) as u32,
            to_7p(q[2] as i32, 0) as u32,
        ],
    )
}
...
pub fn pack_bc7_rgb(block: &mut [u8; 16], pixels: &[Pixel; 16], flags: u32) {
    ...
    for p in pixels.iter() {
        ...
        tg += p[1] as i32;
        tb += p[2] as i32;
        min_r = min_r.min(p[0] as i32);
        max_r = max_r.max(p[0] as i32);
        min_g = min_g.min(p[1] as i32);
        max_g = max_g.max(p[1] as i32);
        min_b = min_b.min(p[2] as i32);
        max_b = max_b.max(p[2] as i32);
    }
    let mean_r = (tr + 8) >> 4;
    let mean_g = (tg + 8) >> 4;
    let mean_b = (tb + 8) >> 4;
    ...
    if (flags & FLAG_USE_TRIVIAL_M6) != 0 && block_max_var < TRIVIAL_BLOCK_VAR {
        ...
        let mut w = [0u8; 16];
        eval_m6_rgb(
            pixels, &mut w, lr as i32, lg as i32, lb as i32, hr as i32, hg as i32, hb as i32, 1, 1,
        );
    ...
    if (flags & FLAG_USE_2SUBSETS) != 0 && block_max_var >= 64 * 16 && ortho_ratio > ORTHO_2SUBSET {
        ...
        for &pat in partitions[..partition_count].iter() {
            ...
            // ── Mode 1 ──
            {
                ...
                pbits = best_pbits_m1(
                    pixels, &mut w, &mut lr, &mut lg, &mut lb, &mut hr, &mut hg, &mut hb, bmask,
                );
    ...
    if (flags & FLAG_USE_DUAL_PLANE) != 0 && block_max_var >= 32 * 16 {
        ...
        if dp_chan >= 0 && dp_chan < 3 {
            ...
            for i in 0..16 {
                rgba[i] = [pixels[i][0], pixels[i][1], pixels[i][2], 255];
            }
...
fn pack_bc7_rgb_mode6_sse(
    ...
    let saxis_r = if xr.abs() >= 1e-6 {
        (xr * 2048.0) as i32
    } else {
        306
    };
    let saxis_g = if xg.abs() >= 1e-6 {
        (xg * 2048.0) as i32
    } else {
        601
    };
    let saxis_b = if xb.abs() >= 1e-6 {
        (xb * 2048.0) as i32
    } else {
        117
    };
    ...
    encode_mode6(
        block, lr as u32, lg as u32, lb as u32, 127, p0, hr as u32, hg as u32, hb as u32, 127, p1,
        &w,
    );
    sse
}
...
pub fn pack_bc7_rgba(block: &mut [u8; 16], pixels: &[Pixel; 16], flags: u32) {
    ...
    for p in pixels.iter() {
        for c in 0..4 {
            total[c] += p[c] as i32;
            mn[c] = mn[c].min(p[c] as i32);
            mx[c] = mx[c].max(p[c] as i32);
        }
    }
    let (tr, tg, tb, ta) = (
        total[0] as f32,
    ...
    let block_max_var4 = *[icov4[0], icov4[4], icov4[7], icov4[9]]
        .iter()
        .max()
        .unwrap();
    ...
    let mut best_sse = {
        ...
        let (mut lo_c, mut hi_c) = (0usize, 0usize);
        for i in 0..16 {
            let dot = pixels[i][0] as i32 * sr
                + pixels[i][1] as i32 * sg
                + pixels[i][2] as i32 * sb
                + pixels[i][3] as i32 * sa
                + i as i32;
            if dot < lo_dot {
                lo_dot = dot;
                lo_c = i;
        ...
        let lo_init = [
            pixels[lo_c][0] as i32,
            pixels[lo_c][1] as i32,
            pixels[lo_c][2] as i32,
            pixels[lo_c][3] as i32,
        ];
        let hi_init = [
            pixels[hi_c][0] as i32,
            pixels[hi_c][1] as i32,
            pixels[hi_c][2] as i32,
            pixels[hi_c][3] as i32,
        ];
        // Run full encoder with p-bit optimisation (FLAG_PBIT_OPT_M6 controls this)
        let use_pbit = (flags & FLAG_PBIT_OPT_M6) != 0;
        pack_bc7_mode6_rgba_full(
            &mut best_block,
            pixels,
            tr,
            tg,
        ...
        if ortho_ratio > ORTHO_2SUBSET {
            ...
            for &pat in partitions[..partition_count].iter() {
                ...
                for s in 0..2 {
                    ...
                    lb[s] = (pixels[li][2] as u32 * 31 + 127) / 255;
                    hb[s] = (pixels[hi_i][2] as u32 * 31 + 127) / 255;
                    la[s] = (pixels[li][3] as u32 * 31 + 127) / 255;
                    ha[s] = (pixels[hi_i][3] as u32 * 31 + 127) / 255;
                }
                let mut w = [0u8; 16];
                let mut pbits4 = [0u32; 4];
                eval_m7(
                    pixels, &mut w, &lr, &lg, &lb, &la, &hr, &hg, &hb, &ha, &pbits4, bmask,
                );
                // P-bit optimisation
                let mut best_p4 = [0u32; 4];
                let mut best_sse7 = u32::MAX;
                for pm_bits in 0u32..16 {
                    ...
                    let mut tw = [0u8; 16];
                    let sse = eval_m7(
                        pixels, &mut tw, &lr, &lg, &lb, &la, &hr, &hg, &hb, &ha, &p, bmask,
                    );
                    if sse < best_sse7 {
                        best_sse7 = sse;
                        best_p4 = p;
...
fn eval_m4_rgb3(
    ...
    let (lr8, lg8, lb8) = (
        from_5(lr as u32) as i32,
        from_5(lg as u32) as i32,
        from_5(lb as u32) as i32,
    );
    let (hr8, hg8, hb8) = (
        from_5(hr as u32) as i32,
        from_5(hg as u32) as i32,
        from_5(hb as u32) as i32,
    );
...
fn eval_m4_rgb2(
    ...
    let (lr8, lg8, lb8) = (
        from(lr as u32) as i32,
        from(lg as u32) as i32,
        from(lb as u32) as i32,
    );
    let (hr8, hg8, hb8) = (
        from(hr as u32) as i32,
        from(hg as u32) as i32,
        from(hb as u32) as i32,
    );
    let (dr, dg, db) = (hr8 - lr8, hg8 - lg8, hb8 - lb8);
...
pub fn encode_mode4(
    ...
    lg: u32,
    lb: u32,
    la: u32, // 5-bit RGB, 6-bit A
    hr: u32,
    hg: u32,
    hb: u32,
    ha: u32,
    w0: &[u8; 16],
    w1: &[u8; 16], // w0=3-bit plane, w1=2-bit plane
    rot_idx: u32,
    index_flag: u32,
) {
...
pub fn detect_dp_channel(icov4: &[i32; 10]) -> i32 {
    ...
    if av > 0.0 {
        let p_ra = if icov4[0] > 0 {
            (icov4[3] as f32).abs() / (rv * av).sqrt()
        } else {
            1.0
        };
        let p_ga = if icov4[4] > 0 {
            (icov4[6] as f32).abs() / (gv * av).sqrt()
        } else {
            1.0
        };
        let p_ba = if icov4[7] > 0 {
            (icov4[8] as f32).abs() / (bv * av).sqrt()
        } else {
            1.0
        };
    ...
    if n_active >= 2 {
        let rg = if has_r && has_g {
            (icov4[1] as f32).abs() / (rv * gv).sqrt()
        } else {
            1.0
        };
        let rb = if has_r && has_b {
            (icov4[2] as f32).abs() / (rv * bv).sqrt()
        } else {
            1.0
        };
        let gb = if has_g && has_b {
            (icov4[5] as f32).abs() / (gv * bv).sqrt()
        } else {
            1.0
        };
...
pub fn pack_mode4_or_5(block: &mut [u8; 16], pixels: &[Pixel; 16], dp_chan: usize) -> u32 {
    ...
    {
        let mut lr = to_7f(px[lo_c][0] as f32) as i32;
        let mut lg = to_7f(px[lo_c][1] as f32) as i32;
        let mut lb = to_7f(px[lo_c][2] as f32) as i32;
        let mut hr = to_7f(px[hi_c][0] as f32) as i32;
        let mut hg = to_7f(px[hi_c][1] as f32) as i32;
        ...
        if let Some((nla, nha)) = ls_fit_1d(16, &wa, &LS_TAB2, px, 3, total[3] as f32) {
            la = rnd(nla).clamp(0, 255);
            ha = rnd(nha).clamp(0, 255);
        }
    ...
    {
        let mut lr = to_5f(px[lo_c][0] as f32) as i32;
        let mut lg = to_5f(px[lo_c][1] as f32) as i32;
        let mut lb = to_5f(px[lo_c][2] as f32) as i32;
        let mut hr = to_5f(px[hi_c][0] as f32) as i32;
        let mut hg = to_5f(px[hi_c][1] as f32) as i32;
        let mut hb = to_5f(px[hi_c][2] as f32) as i32;
        let (mut la, mut ha) = (
            (mn[3] as u32 * 63 + 127) / 255,
            (mx[3] as u32 * 63 + 127) / 255,
        );
        ...
        if let Some((lo, hi)) = ls_fit_3d(
            ...
            lr = to_5f(lo[0]) as i32;
            lg = to_5f(lo[1]) as i32;
            lb = to_5f(lo[2]) as i32;
            hr = to_5f(hi[0]) as i32;
            hg = to_5f(hi[1]) as i32;
            hb = to_5f(hi[2]) as i32;
        }
        ...
        let mut cand = [0u8; 16];
        encode_mode4(
            &mut cand, lr as u32, lg as u32, lb as u32, la, hr as u32, hg as u32, hb as u32, ha,
            &w3, &w2, rot_idx, 1,
        );
        if sse < best_sse {
            best_sse = sse;
...
pub fn determine_unique_pbits_m6(xl: &[f32; 4], xh: &[f32; 4]) -> (u32, u32, [u32; 4], [u32; 4]) {
...
pub fn pack_bc7_mode6_rgba_full(
    ...
    for &(p0, p1) in p_range {
        ...
        let sse = eval_m6_rgba(
            pixels, &mut w, flr, flg, flb, fla, p0, fhr, fhg, fhb, fha, p1,
        );
        if sse < best_sse {
            best_sse = sse;
            encode_mode6(
                block, flr as u32, flg as u32, flb as u32, fla as u32, p0, fhr as u32, fhg as u32,
                fhb as u32, fha as u32, p1, &w,
            );
        }
    }
    best_sse
...
pub fn pack_bc7_mode6_rgb_full(
    ...
    for &(p0, p1) in p_range {
        ...
        if sse < best_sse {
            best_sse = sse;
            encode_mode6(
                block, flr as u32, flg as u32, flb as u32, 127, p0, fhr as u32, fhg as u32,
                fhb as u32, 127, p1, &w,
            );
        }
    }
