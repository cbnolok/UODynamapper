...
use eframe::egui;
use uocf::udd::uddp::{
    reconstruct_stored_size, unpack_codec, unpack_offset40, unpack_type, xxh64_virtual_path, Codec,
    FileKey, UddpReader,
};
...
impl InspectorApp {
    ...
    pub fn load_preview(&mut self, ctx: &egui::Context) {
        ...
        if self.view_mode == ViewMode::Package {
            ...
            // Virtual View
            let result: Option<(VirtualEntry, Option<Vec<u8>>, Option<Vec<u8>>)> = if let Some(idx) = self.selected_virtual_idx {
            ...
            if let Some(res) = result {
                let (ventry, page_data_opt, direct_data_opt): (VirtualEntry, Option<Vec<u8>>, Option<Vec<u8>>) = res;
                match ventry.data {
                    VirtualEntryData::AtlasRect {
                        ...
                        if let Some(page_bytes) = page_data_opt {
                                ...
                                self.atlas_pages.get(&page_index).copied()
                            {
                                ...
                                if let Ok(vram) =
                                    uddconv::bc7::VramTextureData::from_container_bytes(&page_bytes)
                                {
                                    uddconv::bc7::decode_from_vram(
                                        &vram,
                                        uddconv::bc7::RawImageFormat::Rgba8888,
                                    )
                                    .ok()
                                ...
                                .starts_with(&[0xAB, 0x4B, 0x54, 0x58, 0x20, 0x32, 0x30, 0xBB])
                            {
                                if let Ok(k_reader) = ktx2::Reader::new(&page_bytes) {
                                    ...
                                    if k_header.format == Some(ktx2::Format::BC7_UNORM_BLOCK) {
                                        ...
                                        if let Ok(rgba) = uddconv::bc7::decode_bc7_to_rgba8888(
                                            &blocks,
                                            uddconv::bc7::ImageExtent::new(
                            ...
                            if let Some(page) = decoded_page {
                                ...
                                self.atlas_texture_size = Some([page.width as usize, page.height as usize]);
                            }
                        }
                    }
                    ...
                    VirtualEntryData::MapBlock { source_entry_idx } => {
                        ...
                        let preview = self
                            ...
                            .unwrap_or_else(|| {
                                format!("Failed to read map block {}.", ventry.id)
                            });
        ...
        if data.starts_with(&[0xAB, 0x4B, 0x54, 0x58, 0x20, 0x32, 0x30, 0xBB]) {
            if let Ok(k_reader) = ktx2::Reader::new(&data) {
                ...
                if header.format == Some(ktx2::Format::BC7_UNORM_BLOCK) {
                    ...
                    if let Ok(rgba) = uddconv::bc7::decode_bc7_to_rgba8888(
                        &blocks,
                        uddconv::bc7::ImageExtent::new(width, height).unwrap(),
                    ) {
                        self.set_preview_image(
                            ctx,
                            "ktx2_preview",
