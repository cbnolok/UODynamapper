...
impl TerrainDefinitionPackage {
    ...
    pub fn land_tile_ids(&self) -> BTreeSet<u32> {
        self.entries
            .iter()
            .flat_map(|entry| entry.aliases.iter().map(|alias| alias.alias))
            .collect()
    }
    ...
    pub fn texture_selections(&self) -> Vec<(u32, u32)> {
        ...
        for entry in &self.entries {
            ...
            for alias in &entry.aliases {
                selections.entry(alias.alias).or_insert(texture_id);
            }
        }
...
fn classify_texture_path(path: Option<&str>) -> TerrainTextureType {
    match path {
        ...
        Some(value) if value.contains("Data\\TileArtLegacy\\") => TerrainTextureType::TileArtLegacy,
        Some(value) if value.contains("Data\\TileArtEnhanced\\") => TerrainTextureType::TileArtEnhanced,
        Some(value) if value.contains("Data\\Textures\\") => TerrainTextureType::Textures,
